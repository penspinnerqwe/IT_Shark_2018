...
function getPolynom() {
    ...
    return function (x) {
        return args.reduce((summ, current, i) => {
            return summ += current * Math.pow(x, i);
        },null);
    };
}
...
function logger(func, logFunc) {
    let observableFnc = func;
    let loggFnc = logFunc;

    return function () {
        let paramString = Array.from(arguments).reduce((res, current, i, arr) => {
            return res += JSON.stringify(current) + (i < arr.length - 1 ? ',' : '');
        },'')

        loggFnc(observableFnc.name + '(' + paramString + ') starts');
        let res = observableFnc.apply(this, arguments);
        loggFnc(observableFnc.name + '(' + paramString + ') ends');
...
function partialUsingArguments(fn) {
    let scopeParams = Array.from(arguments);
    let calcFnc = scopeParams.shift();

    return function () {
        let localeParams = Array.from(arguments);
        return calcFnc.apply(this, scopeParams.concat(localeParams));
    }
}
