...
function isCreditCardNumber(ccn) {
    let arr = ccn.toString().split('').map( item => { return parseInt(item); });
    let result = 0;
    ...
    for(let i = arr.length % 2 == 0 ? 0 : 1; i < arr.length; i += 2){
        ...
        if( arr[i] > 9 )
            arr[i] -= 9;
        result = arr.reduce( (item, i) => { return item + i; } );        
    }
    return result%10==0;
}
...
function getDigitalRoot(num) {
    let number = num.toString().split('').map( item => parseInt(item) ).reduce( (summ, current) => { return summ + current; });
    return number.toString().split('').map( item => parseInt(item)).reduce( (summ, current) => { return summ + current; });
}
...
function evaluateTicTacToePosition(position) {
    let player1 = position[0][0] !== undefined;
    let player2 = position[0][2] !== undefined;
    ...
    for (let i = 0; i < 3; i++) {
        ...
        player1 = player1 && position[0][0] === position[i][i];
        player2 = player2 && position[0][2] === position[i][2 - i];
    }
